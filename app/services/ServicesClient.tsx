      ...
      <section className="px-5 pb-6">
        <div className="grid grid-cols-2 gap-4">
          {services.map(svc => {
            ...
            return (
              <div key={svc.id} className="relative h-48 rounded-xl overflow-hidden cursor-pointer active:scale-[0.98] transition-transform shadow-lg"
                   onClick={() => openModal(svc)}>
                <Image src={img} alt={svc.name} fill className="object-cover" sizes="(max-width: 768px) 45vw, 200px" />
          ...
          <>
            <div className="modal-gallery p-4 pb-2">
              {(selected.images?.length ? selected.images : ['/logo.png']).map((src,i) => (
                <img key={i} src={imgSizes.modal(src)} alt={selected.name} />
              ))}
            </div>
