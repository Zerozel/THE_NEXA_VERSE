...
export default function ToolsClient({ whatsapp }: { whatsapp: string }) {
  ...
  // FIX: Accurate Math Separation for Service vs Materials
  ...
  // Apply the 30% margin ONLY to the service/labor cost
  const serviceLow = Math.round(servicePrice * (1 - MARGIN) / 100) * 100;
  const serviceHigh = Math.round(servicePrice * (1 + MARGIN) / 100) * 100;
  
  // Add the exact store materials cost to both ends of the final range
  const comboLow  = serviceLow + storeTotal;
  const comboHigh = serviceHigh + storeTotal;
                  ...
                  <div>
                    ...
                    <div className="bg-gradient-to-br from-[#1a1a1a] to-[#111] border border-[#D4AF37]/30 rounded-2xl p-5 mb-3">
                      ...
                      <div className="border-t border-[#333] mt-3 pt-3">
                        ...
                        {/* FIX: Updated UI text to reflect exactly how the math works */}
                        <div className="flex gap-2 mt-2">
                          <span className="bg-[#D4AF37]/20 text-[#D4AF37] text-xs font-bold px-2 py-0.5 rounded-full">±30% Margin on Service</span>
                          <span className="bg-green-900/30 text-green-400 text-xs font-bold px-2 py-0.5 rounded-full">Materials Fixed</span>
                        </div>
                      </div>
                    </div>
