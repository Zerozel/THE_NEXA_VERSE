...
export default function DashboardPage() {
 ...
 async function saveModal() {
    ...
    // 1. Strip out ID, timestamps, AND generated columns like search_vector
    const { id, created_at, updated_at, search_vector, ...restData } = data as any; 
    
    // 2. Attach the images ONLY if it's a service or store item
    const payload = (type === 'service' || type === 'item') 
      ? { ...restData, images: modalImages } 
      : restData;

    // 3. Determine the correct table
    const table = type === 'service' ? 'services' : type === 'item' ? 'store_items' : 'promos';
    ...
    // 5. If successful, clear cache and close modal
    await saveSetting('', {}, table); 
    setModal(null);
    loadAll();
  }
