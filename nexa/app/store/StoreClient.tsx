    ...
    return (
      <div className="bg-white rounded-xl overflow-hidden cursor-pointer shadow active:scale-[0.98] transition-transform relative"
        ...
        <div className="relative w-full h-36 bg-gray-100">
          <Image src={img} alt={item.name} fill className="object-cover" sizes="(max-width:768px) 45vw, 200px" />
        </div>
        ...
        {!search && hotItems.length > 0 && (
          <div className="mb-6">
            ...
            <div className="scroll-row">
              {hotItems.map(item => (
                <div key={item.id} className="flex-shrink-0 w-40"><ProductCard item={item} /></div>
              ))}
            </div>
          </div>
        )}
