      ...
      <section className="py-8 px-5">
        ...
        <div className="scroll-row">
          {initialServices.slice(0, 6).map(svc => {
            const img = imgSizes.card(svc.images?.[0] ?? '/logo.png');
            return (
              <div key={svc.id} className="flex-shrink-0 w-44 h-52 relative rounded-xl overflow-hidden cursor-pointer active:scale-[0.98] transition-transform"
                   onClick={() => bookService(svc)}>
                <Image src={img} alt={svc.name} fill className="object-cover" sizes="176px" />
      ...
      <section className="py-8 px-5 pb-4">
        ...
        <div className="scroll-row">
          {initialStoreItems.slice(0, 6).map(item => {
            const img = imgSizes.card(item.images?.[0] ?? '/logo.png');
            return (
              <div key={item.id} className="flex-shrink-0 w-44 bg-white rounded-xl overflow-hidden cursor-pointer shadow-md active:scale-[0.98] transition-transform"
                ...
                <div className="relative w-full h-36">
                  <Image src={img} alt={item.name} fill className="object-cover" sizes="176px" />
                </div>
                <div className="p-3">
          ...
          <>
            <div className="modal-gallery p-4 pb-2">
              {(modalItem.images?.length ? modalItem.images : ['/logo.png']).map((src, i) => (
                <img key={i} src={imgSizes.modal(src)} alt={modalItem.name} />
              ))}
            </div>
